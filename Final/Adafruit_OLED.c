...
*  SPI.
*/
    // we can directly pass the address of c in
    // we need to establish a place to put the data back from
    unsigned char dummy;

    // Send the string to slave. Chip Select(CS) needs to be
    // asserted at start of transfer and deasserted at the end.

    GPIOPinWrite(GPIOA0_BASE, CS_MASK, 0); // write the chip select pin low
    GPIOPinWrite(GPIOA1_BASE, DC_MASK, 0); // write the DC pin low

    MAP_SPITransfer(GSPI_BASE,&c,dummy,1,
            SPI_CS_ENABLE|SPI_CS_DISABLE);


    GPIOPinWrite(GPIOA0_BASE, CS_MASK, CS_MASK);
...
void writeData(unsigned char c) {
    ...
    GPIOPinWrite(GPIOA0_BASE, CS_MASK, 0); // write the chip select pin low
    GPIOPinWrite(GPIOA1_BASE, DC_MASK, DC_MASK); // write the DC pin low
